...
# the submodule pin moves, then commit the result: builds never run codegen.
set -euo pipefail
...
python3 "$UP/tools/gen_data.py" "$UP/data/en/engine.tvdata" "$UP/src" \
  "$GEN/tvdata.s" "$TMP"/*.o
# Apple clang assembles this file for arm64: translate the ELF flavor to
# Mach-O (section name, underscore-prefixed symbols). See macho_asm.py.
